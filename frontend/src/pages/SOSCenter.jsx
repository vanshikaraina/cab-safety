...
function StatusBadge({ active, anyArmed }) {
  if (active)
    return (
      <div className="sos-status-badge sos-status-active">
  ...
  if (anyArmed)
    return (
      <div className="sos-status-badge sos-status-armed">
        <span className="sos-status-dot" />
        Guards Armed
      </div>
    );
  ...
  return (
    <div className="sos-status-badge sos-status-idle">
      <span className="sos-status-dot" />
      All Clear
    </div>
  );
    ...
    <div className="sos-confirm-overlay">
      <div className="sos-confirm-box">
        ...
        <p className="sos-confirm-sub">
          Triggered via <strong>{trigger}</strong>.
          <br />
          Your contacts are being alerted.
        </p>
        <button className="sos-confirm-dismiss" onClick={onDismiss}>
          Got it
        </button>
      </div>
    </div>
    ...
    <div className="sos-modal-overlay" onClick={onClose}>
      <div className="sos-modal" onClick={(e) => e.stopPropagation()}>
        <div className="sos-modal-header">
          ...
          <button className="sos-modal-close" onClick={onClose}>
            ✕
          </button>
        </div>
          ...
          <div className="sos-relation-chips">
            {relations.map((r) => (
              <button
                key={r}
                className={`sos-chip ${
                  form.relation === r ? "sos-chip-active" : ""
                }`}
        ...
        <div className="sos-modal-footer">
          <button className="sos-btn-ghost" onClick={onClose}>
            Cancel
          </button>
          <button
    ...
    <div className="sos-log-row">
      <div className="sos-log-left">
        ...
        <div className="sos-log-meta">
          <p className="sos-log-location">
            {log.locationLabel || log.location?.address || "Unknown location"}
          </p>
...
export default function SOSCenter() {
    ...
    const load = async () => {
      ...
      try {
        const [pRes, lRes] = await Promise.all([
          ...
          axios
            .get(`${API}/sos/log`, { headers: authHeaders() })
            .catch(() => ({ data: [] })),
        ]);
        ...
        console.error("SOSCenter load error:", err);
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          try {
            ...
            setSosMsg(
              e.response?.data?.message || "❌ SOS failed. Call 100 directly."
            );
          }
  ...
  const { requestPermission: reqShakePerm, permissionState: shakePerm } =
    useShakeSOS({
      armed: shakeArmed,
  ...
  const handleStartCd = (secs) => {
    setCdTotal(secs);
    startCd(secs);
  };

  const handleAddContact = async (form) => {
    const res = await axios.post(`${API}/profile/emergency-contacts`, form, {
      headers: authHeaders(),
    });
  ...
  const handleRemoveContact = async (id) => {
    ...
    try {
      const res = await axios.delete(`${API}/profile/emergency-contacts/${id}`, {
        headers: authHeaders(),
      });
  ...
  const handleClearLog = async () => {
    ...
    try {
      await axios.delete(`${API}/sos/log`, { headers: authHeaders() });
      setSosLog([]);
    } catch {
    ...
    <>
      <div className="sos-page">
          ...
          <div className="sos-hero-right">
            ...
            >
              ...
              <span className="sos-main-inner">
                ...
                <span className="sos-main-sub">
                  {sosActive ? "please wait" : "tap to send"}
                </span>
              </span>
            </button>
          </div>
        ...
        {sosMsg && (
          <div
            className={`sos-toast ${
              sosMsg.startsWith("✅") ? "sos-toast-ok" : "sos-toast-warn"
            }`}
          >
            <span>{sosMsg}</span>
            {sosActive && (
        ...
        {cdActive && (
          <div className="sos-countdown-banner">
            <span>
              ⏱ SOS sending in <strong>{secondsLeft}s</strong>
            </span>
            <button className="sos-countdown-cancel" onClick={cancelCd}>
              Cancel — I'm Safe
            </button>
          </div>
        )}
        ...
        <div className="sos-quickdial">
          {[
            ...
            <a
              key={num}
              href={`tel:${num}`}
              className={`sos-dial-btn sos-dial-${label.toLowerCase()}`}
            >
          ...
          {["contacts", "features", "log"].map((tab) => (
            ...
            >
              ...
              {tab === "features" && (
                <>
                  ⚡ Features
                  {anyArmed && <span className="sos-tab-armed-dot" />}
                </>
              )}
              {tab === "log" && "📋 History"}
            </button>
          ...
          <div className="sos-tab-panel">
            ...
            {contacts.length === 0 ? (
              <div className="sos-empty">
                ...
                <p className="sos-empty-desc">
                  Add at least one person who should be alerted in an emergency.
                </p>
                <button
                  className="sos-btn-primary"
                  onClick={() => setShowAddModal(true)}
          ...
          <div className="sos-tab-panel">
            <div className="sos-section-header">
              <div>
                ...
                <p className="sos-section-desc">
                  Multiple ways to trigger SOS without unlocking your screen.
                </p>
              </div>
            </div>
            ...
            <div className="sos-features-notice">
              <span>ℹ️</span>
              <p>
                These guards work while this browser tab is <strong>open</strong>{" "}
                instead of when the browser is fully closed. For best protection,
                keep this page open during your ride.
              </p>
            </div>
          ...
          <div className="sos-tab-panel">
            <div className="sos-section-header">
              <div>
                ...
                <p className="sos-section-desc">
                  All past alerts, sorted newest first.
                </p>
              </div>

              {sosLog.length > 0 && (
                <button
                  className="sos-btn-ghost sos-btn-danger"
            ...
            {sosLog.length === 0 ? (
              <div className="sos-empty">
                ...
                <p className="sos-empty-desc">
                  Your alert history will appear here. Stay safe out there.
                </p>
              </div>
            ) : (
